export class Input {
  ...
  constructor(input_json: { [key: string]: any }, table_names: string[]) {
    ...
    this.teachers = this.GetTeachers(
      input_json[table_names[1] + "data"],
      input_json[table_names[1] + "columns"],
    );
    this.rooms = this.GetRooms(
      input_json[table_names[2] + "data"],
      input_json[table_names[2] + "columns"],
    );
    this.student_groups = this.GetStudentGroups(
      input_json[table_names[3] + "data"],
      input_json[table_names[3] + "columns"],
    );
    this.classes = this.GetClasses(
      input_json[table_names[0] + "data"],
      input_json[table_names[0] + "columns"],
    );
  }
...
export class Class {
  ...
  constructor(
    ...
    for (const teacher of row[columns[2]["accessor"]].split(",")) {
      this.teacher_indexes.push(
        teachers.findIndex((val) => val.name == teacher),
      );
    }
    this.room_candidates_indexes = new Array<number>();
    for (const room of row[columns[3]["accessor"]].split(",")) {
      this.room_candidates_indexes.push(
        rooms.findIndex((val) => val.name == room),
      );
    }
    this.students_group_indexes = new Array<number>();
    for (const student_group of row[columns[4]["accessor"]].split(",")) {
      this.students_group_indexes.push(
        students_groups.findIndex((val) => val.name == student_group),
      );
    }
