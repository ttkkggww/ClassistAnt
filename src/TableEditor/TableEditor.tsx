...
const TableEditor :React.FC<TableEditorProps> = (props:TableEditorProps) => {
  ...
  const defaultTableData = {columns:[
    {
      Header: "NoData",
      accessor: "NoData",
    }
  ...
  useEffect(() => {
    invoke<any>("handle_get_table",{tableType:props.tableName}).then(
      (res)=>{
    ...
    <div className={styles.field}>
      <div style={{ height: "100vh", overflow: "scroll" }}>
        <div style={{ height: "100%" }}>
            <Table columns={table.columns} data={table.data} width={660} tableName={props.tableName} />
        </div>
      </div>
    </div>
  );
