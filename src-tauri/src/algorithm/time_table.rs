...
pub fn convert_solver_to_timetable(solver: &ACOSolver) -> Result<TimeTable, Box<dyn Error>> {
    ...
    for (class_id, &[room_id, period_id]) in best_ant.get_corresponding_crp().iter().enumerate(){
        ...
        cells[room_id as usize * solver.parameters.num_of_periods as usize + period_id as usize] = Cell::ActiveCell(ActiveCell{
            id: id as usize,
...
fn calc_pheromone_color(solver: &ACOSolver,class_id:usize,room_id:usize,period_id:usize) -> String {
    let mut res = String::from("FFFFFF");
    if let Some(ant) = solver.get_best_ant(){
        let (rp_v,prov_v) = ant.calc_prob_from_v_igunore_visited(class_id, solver.colony.get_graph());
